import createSvgIcon from "@mui/joy/utils/createSvgIcon";

const BookingIcon = createSvgIcon(
  <svg
    xmlns="http://www.w3.org/2000/svg"
    version="1.1"
    width="512"
    height="512"
    x="0"
    y="0"
    viewBox="0 0 512 512"
    fill="currentColor"
  >
    <g>
      <path
        d="M406.713 15.5H105.288C47.232 15.5 0 62.732 0 120.788c0 58.337 47.723 105.288 104.66 105.288h47.29v-82.494c0-36.944 30.055-67 66.998-67h.146c36.865.079 66.856 30.135 66.856 67v82.494h120.763c58.056 0 105.287-47.232 105.287-105.288S464.769 15.5 406.713 15.5zm29.794 90.606-60 60c-5.857 5.858-15.355 5.859-21.213 0l-30-30c-5.858-5.858-5.858-15.355 0-21.213 5.857-5.858 15.355-5.858 21.213 0l19.394 19.393 49.394-49.393c5.857-5.858 15.355-5.858 21.213 0 5.857 5.858 5.857 15.356-.001 21.213z"
        opacity="1"
        data-original="#000000"
      ></path>
      <path
        d="m352.514 268.29-96.563-10.037V143.581c0-20.404-16.517-36.956-36.921-37-20.465-.044-37.079 16.534-37.079 37V335.5h-.336L145.092 305c-15.405-12.864-38.393-10.452-50.793 5.329-11.891 15.134-9.595 36.979 5.182 49.31l74.133 61.861H391.95V312.944c0-22.702-16.909-41.848-39.436-44.654zM181.95 481.5c0 8.284 6.716 15 15 15h180c8.284 0 15-6.716 15-15v-30h-210z"
        opacity="1"
        data-original="#000000"
      ></path>
    </g>
  </svg>,
  "EicrOutlined"
);
export default BookingIcon;
